...
Management classes allows cobbler to function as an configuration management system. Management classes provide a collection of management resources as defined in /etc/cobbler/resources/*. Cobbler currently supports the following resource types: (repositories, groups, users, mounts, packages, directories, files, services).

Resources are executed in the order listed above. Service resources have an weight option to ensure services can be stopped, started, and restarted granular. 

B<cobbler mgmtclass add --name=string --comment=string [--repositories=list] [--groups=list] [--users=list] [--mounts=list] [--packages=list] [--directories=list] [--files=list] [--services=list]>
...
Specifies a list of repository resources required by the management class.

=item groups

Specifies a list of group resources required by the management class.

=item users

Specifies a list of user resources required by the management class.

=item mounts

Specifies a list of mount resources required by the management class.
...
Specifies a list of directory resources required by the management class.
...
Specifies a list of service resources required by the management class.
...
B<cobbler distro|profile|system|repo|image list>

B<cobbler report>

B<cobbler distro|profile|system|repo|image report --name=[object-name]>
...
B<cobbler distro|profile|system|repo|image edit --name=string [parameterlist]>

=head2 COPYING

Objects can also be copied:

B<cobbler distro|profile|system|repo|image copy --name=oldname --newname=newname>

=head2 RENAMING

Objects can also be renamed, as long as other objects don't reference them.

B<cobbler distro|profile|system|repo|image rename --name=oldname --newname=newname>
...
Resources are the lego blocks of configuration management. Resources are grouped together via Management Classes, which are then linked to a system. Cobbler supports eight (8) resource types. Resources are configured in the order listed below.
...
=head2 META ATTRIBUTES

All resources support the following meta attributes.
...
A list of services to reload when a change to this resource is made. Services are restarted using the service weighted system. Services with a higher weight are restarted first.
...
=head3 Examples
 ...
 template = /var/lib/cobbler/templates/httpd.conf.tmpl
 owner = root
 group = root
 mode = 644
 services = httpd
 ...
 group = qpidd
 mode = 644
 services = openais,qpidd


=head2 REPOSITORY RESOURCES

Repository resources define native package repos used to install packages. Repositories are created using the cheetah template assigned via the template attribute. File attributes such as mode, owner, and group are automatically set based on the target OS. Repositories are managed in /etc/cobbler/resources/repositories
...
Which cheetah template to use for this repository [Required]

=item gpgkey

Path to the gpg-key for this repository.

=head3 Examples

 [updates]
 template = /var/lib/cobbler/templates/updates.repo.tmpl
 gpgkey = /var/lib/cobbler/gpgkeys/updates.key
 action = create

=head2 GROUP RESOURCES

Group resources define Unix system groups. Group resources are managed in /etc/cobbler/resources/groups
...
=head3 Examples
 ...
 name = nagios
 gid = 600
 members = nagios,root,monitoradmin
 action = create


=head2 USER RESOURCES

User resources define Unix system accounts. User resources are managed in /etc/cobbler/resources/users
...
=head2 DIRECTORY RESOURCES

Directory resources are managed in /etc/cobbler/resources/directories
...
The group owner of the directory (string or id) [Required]

=item mode

Permission mode (as in chmod) [Required]

=item owner

The owner for the directory [Required]

=item path

The path that should be created or removed. [Required]
...
=head2 MOUNT RESOURCES

Mount resources describe mounted filesystems configured via fstab. Mount resources are managed in /etc/cobbler/resources/mounts
...
Mount the mount-point, and add it to /etc/fstab. [Default]

=item remove

Unmount the mount-point, and remove it from /etc/fstab. 
...
Used for by the dump(8) command to determine which filesystems need to be dumped. If not present dump will assume that the filesystem does not need to be dumped.

=item fs_passno

Used by the fsck(8) program to determine the order in which filesystem checks are done at reboot time.Default value = 2

=head3 Examples

 [data]
 fs_spec = 10.0.1.15:/data
 fs_file = /data
 fs_vfstype = nfs
 fs_mntops = ro,timeo=30
 fs_freq = 0
 fs_passno = 0
 action = create
...
Package resources are managed in /etc/cobbler/resources/packages
...
The package url to install from.
...
File resources are managed in /etc/cobbler/resources/files
...
=head2 SERVICE RESOURCES

Service resources have an weight option to ensure services can be stopped, started, and restarted as defined by the end user. This allows features such as resource dependencies and service restarts/reloads based on resource changes to work. Service resources are managed in /etc/cobbler/resources/services.
...
Name of the service to manage via /sbin/service command

=item weight

A number between 1 - 100, determines the order of service stop, start, restart, and reload.

=item reload_support

Weather the resource supports the reload argument used by the native service manager. This option is useful for resources that do not require a restart to reload configuration files.
