...
Management classes allows cobbler to function as an configuration management system. Management classes provide a collection of management resources as defined in /etc/cobbler/resources/*. Cobbler currently supports the following resource types: (repositories, groups, users, mounts, packages, directories, files).

Resources are executed in the order listed above.

B<cobbler mgmtclass add --name=string --comment=string [--repositories=list] [--groups=list] [--users=list] [--mounts=list] [--packages=list] [--directories=list] [--files=list]>
